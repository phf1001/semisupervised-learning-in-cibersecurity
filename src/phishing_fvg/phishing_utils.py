...
# -*-coding:utf-8 -*-
"""
@File    :   phishing_utils.py
@Time    :   2023/03/30 21:02:24
...
def is_foreign(self_url, url):
    ...
    return (
        not is_empty(url)
        and not is_in_local(url)
        and urlparse(self_url).netloc != urlparse(url).netloc
    )
...
def is_empty(url):
    ...
    return (
        url == ""
        or url[0] == "#"
        or bool(re.match("[Jj]ava[Ss]cript::?void\(0\)", url))
    )
...
def get_response_code(url, headers, proxies):
    ...
    """
    return requests.get(
        url, headers=headers, proxies=proxies, allow_redirects=False, timeout=15
    ).status_code
...
def get_bin_source_code(
    url, headers, proxies, fichero=get_data_path() + os.sep + "html_dump"
):
