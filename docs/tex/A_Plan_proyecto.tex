...
En este proyecto, se han utilizado los <<puntos de historia>> como unidad de medida. Es destacable que se ha establecido la equivalencia de un punto de historia a una hora real de trabajo.
...
\begin{itemize}
	...
	\begin{figure}[h]
		\caption[\textit{Sprint} 08: \textit{burndown report}]{\textit{Burndown Report Sprint 08}}
		\centering
...
\begin{itemize}
	...
	\begin{figure}[h]
		\caption[\textit{Sprint} 16: \textit{burndown report}]{\textit{Burndown Report Sprint 16}}
		\centering
...
\subsubsection{\textit{Sprint 20: Sumac}}
\begin{itemize}
	\item \textbf{\textit{Planning meeting}}
	...
	\begin{figure}[h]
		\caption[\textit{Sprint} 20: \textit{burndown report}]{\textit{Burndown Report Sprint 20}}
		\centering
		\includegraphics[width=\textwidth]{../img/anexos/bdr/s20_bdr}
	\end{figure}
	
	Como se puede comprobar, durante este \textit{sprint} no se han conseguido cerrar todos los \textit{issues}. Además, se realizó una estimación muy inferior de tiempo requirido. Se calculó que para cerrar todos los \textit{issues} había que dedicar 33 horas, cuando en realidad se han dedicado 39 y no se ha conseguido completar lo previsto. Queda pendiente la realización de pruebas y completar los diagramas durante el siguiente \textit{sprint}.
	
	\item \textbf{\textit{Sprint review meeting}}
	
	Esta reunión fue particularmente extensa ya que se planteó la distribución temporal de todas las tareas restantes para completar el proyecto. Además, también se realizaron ciertas correcciones relacionadas con los diagramas y se comentó un problema respectivo a la internacionalización.
\end{itemize}


\subsubsection{\textit{Sprint 21: Advieh}}
\begin{itemize}
	\item \textbf{\textit{Planning meeting}}
	...
	\begin{figure}[h]
		\caption[\textit{Sprint} 21: \textit{burndown report}]{\textit{Burndown Report Sprint 21}}
		\centering
		\includegraphics[width=\textwidth]{../img/anexos/bdr/s21_bdr}
	\end{figure}
	
	En este \textit{sprint} se han cerrado los \textit{issues propuestos}. El tiempo invertido ha sido superior que en el \textit{burndown report}, calculándose unas 50 horas reales. 
	
	El motivo es que la documentación ha requerido mayor tiempo del estimado, además del despliegue en Docker (que no se tuvo en cuenta planeando el \textit{sprint}, pero se ha realizado por considerarse adecuado), la codificación de \textit{scripts} y la solución para llenar la base de datos.
	
	\item \textbf{\textit{Sprint review meeting}}
	
	Durante esta \textit{sprint review meeting}, principalmente se recibió \textit{feedback} respecto a la experimentación de aprendizaje supervisado contra semisupervisado. Se propuso la realización de validación cruzada repetida, lanzando los experimentos en máquinas pertenecientes al grupo de investigación ADMIRABLE\footnote{Disponible en: \url{https://www.ubu.es/advanced-data-mining-research-and-bioinformatics-learning-admirable}}.
\end{itemize}

\subsubsection{\textit{Sprint 22: Cardamom}}
\begin{itemize}
	\item \textbf{\textit{Planning meeting}}
	...
	\begin{figure}[h]
		\caption[\textit{Sprint} 22: \textit{burndown report}]{\textit{Burndown Report Sprint 22}}
		\centering
		\includegraphics[width=\textwidth]{../img/anexos/bdr/s22_bdr}
	\end{figure}
	
	En este \textit{sprint} se ha logrado cerrar todos los \textit{issues} pendientes, habiendo hecho una estimación correcta del tiempo requerido. Se calculó que se iban a invertir 47 horas, cuando el tiempo real ha sido de 48.
	
	A diferencia de otros \textit{sprints} y en contra de los estándares impuestos por Scrum, debido a la cercana fecha de entrega, se ha permitido introducir \textit{issues} al \textit{backlog} durante el \textit{sprint} (principalmente de corrección de \textit{bugs} encontrados por el tutor).
	
	\item \textbf{\textit{Sprint review meeting}}
	
	Durante esta reunión se ha recibido algún detalle en forma de retroalimentación (pequeñas correcciones en los anexos y la memoria).
\end{itemize}


\subsubsection{\textit{Sprint 23: Cumin}}
\begin{itemize}
	\item \textbf{\textit{Planning meeting}}
	
	En esta reunión se estableció como objetivo la finalización del proyecto, cerrando pequeños detalles pendientes de documentación e implementación. El contenido del \textit{sprint backlog} se muestra a continuación:
	
	\begin{enumerate}
		\item Comparación y experimentación en la detección de \textit{phishing}: recoger los resultados de la validación cruzada repetida (requiere un tiempo de ejecución cercano a las dos semanas) y documentar las conclusiones obtenidas.
		\item Memoria: redactar el capítulo 7 (conclusiones y líneas futuras), repasar todo el documento y actualizar los enlaces incluídos. Revisar las imágenes, imprimir y encuadernar.
		\item Anexos: completar el manual de usuario y añadirlo, además, a la \textit{wiki}\footnote{Disponible en \url{https://github.com/phf1001/semisupervised-learning-in-cibersecurity/wiki}.} de GitHub. Revisar los diagramas y redimensionar los que se lean peor.
		\item Memorias USB: preparar las memorias USB entregables para el tribunal.
		\item Tutoriales: grabar tutoriales para YouTube, editarlos y subirlos al canal. Crear animación inicial para los vídeos y fondos de pantalla para escritorio y dispositivos móviles. Actualizar los enlaces en los ficheros \texttt{README.md} y en la documentación.
		\item \textit{Web}: solucionar pequeños detalles encontrados durante la grabación de los vídeos.
		\item Heroku: diseñar y establecer una pantalla personalizada para cuando suceda un error de aplicación. Solucionar problemas relacionados con el \textit{iframe} y los vínculos. Reestablecer la base de datos creando credenciales para el tribunal. Actualizar certificados SSL.
		\item Docker: realizar un último \textit{push} a DockerHub y solucionar pequeños detalles en los \textit{scripts} (puertos).
		
		
	\end{enumerate}
	\item \textbf{Marcas temporales}		
	
	El \textit{sprint} se ha desarrollado entre el 29 de mayo y el 11 de junio del 2023.
	
	\item \textbf{\textit{Burndown Report}}
	
	\begin{figure}[h]
		\caption[\textit{Sprint} 23: \textit{burndown report}]{\textit{Burndown Report Sprint 23}}
		\centering
		\includegraphics[width=\textwidth]{../img/anexos/bdr/s23_bdr}
	\end{figure}
	
	En este \textit{sprint} se ha logrado cerrar todos los \textit{issues} pendientes, habiendo hecho una estimación correcta del tiempo requerido.
	
	\item \textbf{\textit{Sprint review meeting}}
	
	En esta reunión se sugirieron pequeñas correcciones en la memoria y los anexos, además de su aprobación y firma. También se comenzó a plantear la presentación del trabajo, proponiendo distintas estructuras y valorando las ventajas e inconvenientes de cada una.
\end{itemize} 
...
La viabilidad de un proyecto \textit{software} es la capacidad que tiene este para ser realizado de manera exitosa y cumpliendo los objetivos propuestos. Para determinar si este proyecto es factible, se va a evaluar la viabilidad económica (sección~\hyperref[s:ve]{A.3.1}) y legal (sección~\hyperref[s:vl]{A.3.2}).
...
Se estima que el tiempo real invertido en el proyecto por parte de la desarrolladora ha sido equivalente a 650 horas repartidas a lo largo de 8.5 meses. Teniendo en cuenta que el sueldo base promedio de un programador \textit{junior} en España equivale a 21\,496€ brutos anuales\footnote{Fuente: \url{https://es.indeed.com/career/programador-junior/salaries}} y que el tiempo anual trabajado en una jornada completa es aproximadamente 1\,820 ~horas\footnote{Fuente: \url{ https://www.expansion.com/diccionario-juridico/jornada-laboral.html}}, el salario medio bruto por hora es, aproximadamente, 11.82€. Por ello, el salario bruto del programador equivale a $650\text{ horas} \times 11\text{.}82\frac{\text{€}}{hora} = 7\,677\text{.}14\text{€}$ (aproximadamente 903.2$\frac{\text{€}}{mes}$).

A esta cantidad se le deben añadir los impuestos que ha de pagar la empresa por el empleado\footnote{Fuente:~\url{https://www.seg-social.es/wps/portal/wss/internet/Trabajadores/CotizacionRecaudacionTrabajadores/10721/10957/9932/4315}}. En concreto, un $23$.$6\%$ de contingencias, un $5$.$50\%$ de desempleo, un $0$.$20\%$ de FOGASA, un $0$.$60\%$ de formación profesional y un $1$.$50\%$ por accidentes de trabajo. Por lo tanto, la empresa debe pagar por este trabajador 1\,316.62$\frac{\text{€}}{mes}$. Esta cifra proviene de despejar en la ecuación~\ref{eqn:sueldo-patri}.

\begin{equation}\label{eqn:sueldo-patri} \frac{903\text{.}2\frac{\text{€}}{mes}}{1 - (0\text{.}236 + 0\text{.}055 + 0\text{.}002 + 0\text{.}006 + 0\text{.}0150)} = 1\,316\text{.}62 \frac{\text{€}}{mes}\end{equation}


Respecto al tutor del proyecto, suponiendo que cobra de media 38\,707.62€ brutos al año\footnote{Fuente: \url{https://www.ubu.es/sites/default/files/portal_page/files/pdi_funcionario_ano_2022.pdf}}, se calcula que el sueldo bruto por hora es de 21.27€. Calculando que el tutor dedica una media de 3 horas por semana al proyecto, debería cobrar 277.23€ brutos al mes. Sumando los impuestos, la empresa debería pagar 404.12$\frac{\text{€}}{mes}$ por los servicios prestados (ecuación~\ref{eqn:sueldo-alvar}).

\begin{equation}\label{eqn:sueldo-alvar} \frac{277\text{.}23 \frac{\text{€}}{mes}}{1 - (0\text{.}236 + 0\text{.}055 + 0\text{.}002 + 0\text{.}006 + 0\text{.}0150)} = 404\text{.}12 \frac{\text{€}}{mes}\end{equation}
...
\begin{table}[p]
	...
	\begin{tabular}{l r r}
		...
		\midrule
		Sueldo desarrolladora & 903.2 & 1316.62 \\
		Sueldo \textit{product owner} & 277.23 & 404.12 \\
		\midrule
		\textbf{Total en 8.5 meses} & 10\,034.25 & 14\,626.29 €\\
		\bottomrule
	\end{tabular}
...
\item\textbf{Costes \textit{software}:} la causa de este desembolso es la necesidad de adquirir licencias. El precio que se muestra es el acorde a nivel empresarial por año. Por lo tanto, el coste total para la empresa a nivel de \textit{software} se muestra en la tabla~\ref{a:costes-software}. Es destacable que el dólar estadounidense ha sido convertido a 0.93€ (18 de Mayo del 2023, 16:10).
...
\begin{table}[p]
	...
	\begin{tabular}{l r r}
		...
		\midrule
		Oficina & 0 & 0\\
		Fibra Óptica & 0.048 & 31.2 \\
		Luz & 0.056 & 36.4\\
		Calefacción & 0.27 & 175.5\\
		Agua & 0.03 & 19.5 \\
		\midrule
		\textbf{Total} & 0.404 & 262.6 \\
		\bottomrule
	\end{tabular}
...
\begin{table}[p]
	...
	\begin{tabular}{l r}
		...
		\midrule
		Empleados & 14\,626.29 \\
		\textit{Hardware} & 677.42 \\
		\textit{Software} & 1\,506.63\\
		Otros & 262.6\\
		\midrule
		\textbf{Total} & 17\,072.94 \\
		\bottomrule
	\end{tabular}
...
Un beneficio es aquella ventaja obtenida como resultado de un proyecto y representa el incremento del valor económico (o de los recursos) para una entidad.
...
La comparativa entre licencias\footnote{Tabla realizada con \url{https://joinup.ec.europa.eu/collection/eupl/solution/joinup-licensing-assistant/jla-find-and-compare-software-licenses}} se muestra la tabla~\ref{a:permisos}. El único detalle que no se muestra en la imagen es que la BSD-2-Clause License es más permisiva que la BSD-3-Clause License~\cite{BSD}. Como se puede comprobar, el orden de restrictividad (de más permisiva a más restrictiva) es el siguiente:
...
\begin{table}[p]
	...
	\begin{tabularx}{\linewidth}{ p{0.3\textwidth} p{0.15\textwidth} p{0.55\textwidth} }
		\toprule
		\textbf{Dependencia} & \hfil\textbf{Versión} & \textbf{Licencia} \\
		\midrule
		\texttt{Argon Dashboard} & \hfil2022 & MIT License\\
		\texttt{Babel} & \hfil2.12.1 & MIT License\\
		\texttt{beautifulsoup4} & \hfil4.11.1 & MIT License \\
		\texttt{email-validator} & \hfil1.1.3 & The Unlicense \\
		\texttt{Flask} & \hfil2.0.1 & BSD-3-Clause License\\
		\texttt{flask-babel} & \hfil3.1.0 & BSD License\\
		\texttt{Flask-Bootstrap} & \hfil3.3.7.1 & Apache Software License (2.0)\\
		\texttt{Flask-Login} & \hfil0.6.0 & MIT License \\
		\texttt{Flask-Migrate} & \hfil3.1.0 & MIT License \\
		\texttt{Flask-SQLAlchemy} & \hfil2.5.1 & BSD-3-Clause License \\
		\texttt{Flask-WTF} & \hfil0.15.1 & BSD-3-Clause License \\
		\texttt{gunicorn} & \hfil20.1.0 & MIT License \\
		\texttt{Jinja2} & \hfil3.1.2 & BSD-3-Clause License \\
		\texttt{jsonschema} & \hfil4.17.3 & MIT License \\
		\texttt{lxml} & \hfil4.9.2 & BSD-3-Clause License \\
		\texttt{nltk} & \hfil3.8.1 & Apache Software License (2.0)\\
		\texttt{node} & \hfil1.2.1 & BSD-2-Clause License\\
		\texttt{numpy} & \hfil1.23.3 & BSD-3-Clause License \\
		\texttt{pandas} & \hfil1.4.3 & BSD-3-Clause License \\
		\texttt{psycopg2-binary} & \hfil2.9.6 & GNU Lesser General Public (LGPLv2) \\
		\texttt{PySocks} & \hfil1.7.1 & BSD License \\
		\texttt{pytest} & \hfil7.2.0 & MIT License \\
		\texttt{python-decouple} & \hfil3.4 & MIT License \\
		\texttt{regex} & \hfil2023.3.23 & Apache Software License  \\
		\texttt{requests} & \hfil2.28.2 & Apache Software License (2.0) \\
		\texttt{scikit-learn} & \hfil1.1.2 & BSD-3-Clause License\\
		\texttt{scipy} & \hfil1.9.3 & BSD License \\
		\texttt{SQLAlchemy} & \hfil1.4.23 & MIT License \\
		\texttt{sslearn} & \hfil1.1.0 & BSD-3-Clause License \\
		\texttt{LAMDA-SSL} & \hfil1.0 & MIT License \\
		\texttt{threadpoolctl} & \hfil3.1.0 & BSD-3-Clause License \\
		\texttt{tld} & \hfil0.13 & GNU General Public License (GPLv2)\\
		\texttt{urllib3} & \hfil1.26.5 & MIT License \\
		\texttt{Werkzeug} & \hfil2.1.0 & BSD-3-Clause License \\
		\texttt{WTForms} & \hfil2.3.3 & BSD-3-Clause License \\
		\bottomrule
	\end{tabularx}
...
\begin{table}[p]
	...
	\caption[Licencias: permisos]{Acciones permitidas en función de las licencias. Realizada con: \url{https://joinup.ec.europa.eu/collection/eupl/solution/joinup-licensing-assistant/jla-find-and-compare-software-licenses}}
	\label{a:permisos}
\end{table}

A pesar de que todas las licencias son \textit{open source}, hay algunas cuyo tipo es \textit{copyleft}\footnote{En las licencias \textit{copyleft}, el código deriva de código \textit{open source} y hereda sus licencias.} (LGLP y GLP~\cite{oslicenses}). Por lo tanto, se ha decidido aplicar este tipo de licencias al \textit{software} desarrollado, solo que utilizando las nuevas versiones (GLPv3 en lugar de GLPv2).
...
Como se ha expuesto en el desarrollo de la memoria, para obtener un vector de características únicamente se necesita realizar una petición al servidor principal para extraer el código \texttt{HTML} de la página. Debido a que esto no se clasifica como mala práctica y no se almacena ningún tipo de dato \textit{scrapeado}, se concluye que este método no supone una amenaza legal para el proyecto.
