...
\section{Experimentación con algoritmos de aprendizaje semisupervisado}
...
\begin{figure}[h]
	...
	\centering
	\includegraphics[scale=0.6]{../img/memoria/5_coforest_score-iteraciones_individual}
...
\begin{figure}[h]
	...
	\centering
	\includegraphics[scale=0.7]{../img/memoria/5_coforest_score-porcentaje_entrenamiento}
...
Nuevamente, se ha querido evaluar la evolución de la \textit{accuracy} del modelo en función de la iteración del entrenamiento en la que se encuentre. En este caso, el \textit{tri-training} utiliza como estimadores base un \textit{naive-bayes} gaussiano, un árbol de decisión y un \textit{K-neighbors}. Al igual que en el \textit{co-forest}, los resultados representados son la media de 10 experimentos realizados mediante validación cruzada y se pueden observar en la gráfica~\ref{5_tritraining_score-iteraciones}.

\begin{figure}[h]
	\caption[\textit{Tri-training}: Variación del \textit{score} durante el entrenamiento]{Gráfica que representa la \textit{accuracy} del modelo en función de la iteración del entrenamiento en la que se encuentre.}
	\centering
	\includegraphics[scale=0.7]{../img/memoria/5_tritraining_score-iteraciones}
	\label{5_tritraining_score-iteraciones}
\end{figure}

Como se puede comprobar, el modelo mejora los resultados iniciales (exactitud obtenida en la iteración $0$, cuando todavía no se ha comenzado el algoritmo de entrenamiento semisupervisado) en la mayoría de los casos, siendo especialmente notable en los \textit{datasets} <<Vino>> y <<Dígitos>>.

\subsubsection{Tiempo de entrenamiento}

Nuevamente se ha trabajado con el \textit{dataset} que contiene un mayor número de datos y los resultados obtenidos se pueden observar en la gráfica~\ref{5_tritraining_tiempo-instancias}. De nuevo, se sigue un crecimiento aproximadamente lineal con una alta velocidad (debido en parte a que este \textit{dataset} no realiza un número elevado de iteraciones durante el entrenamiento).

\begin{figure}[h]
	\caption[\textit{TriTraining}: Variación del tiempo de entrenamiento en función del número de instancias]{Gráfica que representa la evolución del tiempo de entrenamiento para el \textit{tri-training} en función del número de instancias utilizadas.}
	\centering
	\includegraphics[scale=0.7]{../img/memoria/5_tritraining_tiempo-instancias}
	\label{5_tritraining_tiempo-instancias}
\end{figure}

\subsubsection{Datos de entrenamiento}

En este caso, se quiere evaluar el desempeño del \textit{tri-training} en función del porcentaje del \textit{dataset} que se utilice para el entrenamiento (posteriormente este conjunto será dividido en $L$ ($20\%$) y $U$ ($80\%$)). Los resultados mostrados son la media de 10 experimentos y se observan en la gráfica~\ref{5_tritraining_score-porcentaje_entrenamiento}. En este caso, un buen compromiso sería utilizar un $80\%$ de los datos, ya que por lo general a mayor número de instancias utilizadas durante el entrenamiento mejor resultado.

\begin{figure}[h]
	\caption[\textit{Co-forest}: Variación del \textit{score} en función del porcentaje de datos]{Gráfica que representa la exactitud media del \textit{tri-training} en función del porcentaje del \textit{dataset} utilizado en la fase de entrenamiento.}
	\centering
	\includegraphics[scale=0.7]{../img/memoria/5_tritraining_score-porcentaje_entrenamiento}
	\label{5_tritraining_score-porcentaje_entrenamiento}
\end{figure}
