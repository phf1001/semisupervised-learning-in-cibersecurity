...
Un ataque web es aquel en el que el atacante controla el sitio visitado (por ejemplo: www.atacante.com)~\cite{apuntesCybersec}. Cada vez que se visita una web, el navegador (ya sea en ventana o pestaña) carga el contenido, lo renderiza y responde a los eventos. Esto no deja de ser una oportunidad para los atacantes, que pueden utilizar las etiquetas HTML (por ejemplo, dentro de una imagen) para comunicarse con otros sitios o suplantar webs legítimas. Además, también pueden utilizar \textit{scripts} para comunicarse con lugares externos (\textit{remote scripting}).

Otro factor a tener en cuenta es que se pueden explotar las vulnerabilidades de los navegadores para atacar~\cite{apuntesCybersec}. EN PROCESO, dibujos abajo de momento...

\begin{itemize}
	\item \textbf{SQL \textit{Injection}}:
	...
	\begin{figure}[h]
		...
		\label{img:xss}
		\centering
		\includegraphics[scale=0.5]{../img/memoria/3_xss}
	\end{figure}
\end{itemize}
