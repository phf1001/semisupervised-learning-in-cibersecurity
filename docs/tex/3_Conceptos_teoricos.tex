...
En estos algoritmos, varios clasificadores son entrenados iterativamente utilizando datos etiquetados y añadiendo las predicciones (resultados) más confiables al conjunto para ser utilizadas en las siguientes iteraciones. Para que los clasificadores sean capaces de generar información distinta, generalmente se divide el conjunto de entrada según alguna característica (no siendo estrictamente necesario). El \textit{co-forest}, algoritmo protagonista de este proyecto, pertenece a esta categoría.
...
Se define \textit{ensemble} como un conjunto de modelos de \textit{machine learning} donde cada estimador base genera una predicción individual que se combina con el resto para generar una salida única.
	
\subsection{\textit{Bagging}}

Este modelo se compone de un conjunto de estimadores base utilizados en paralelo. Por lo tanto, el resultado es el promedio (o más popular) de las salidas de los modelos simples. Para entrenar cada estimador base se utiliza \textit{bootstrapping}, que es una técnica de muestreo que genera un subconjunto de datos seleccionando aleatoriamente muestras de un conjunto mayor permitiendo la repetición. Es decir, los clasificadores son entrenados con un subconjunto del total de los datos etiquetados.
	
Uno de los métodos más populares es el \textit{random forest}, que obtiene resultados certeros debido a la aleatoriedad introducida. Cuando se devuelve una etiqueta, es el resultado de una votación realizada por todos los árboles del conjunto. Además de utilizar \textit{bootstrapping}, en el entrenamiento individual de cada árbol, únicamente se <<ven>> algunos atributos (no el total) para introducir mayor aleatoriedad.
	
\subsection{\textit{Boosting}}

Si el anterior modelo utiliza los clasificadores base <<en paralelo>>, este lo haría <<en serie>>. Es decir, hay un orden secuencial: los estimadores dependen del resultado anterior y tratan de compensar el error que se haya podido cometer.

TO-DO: Dibus de los modelos y meter los dos que faltan
...
El denominado \textit{co-forest} es un método de envoltura que permite construir un \textit{ensemble} de árboles de decisión. Este método está basado en el \textit{random forest} y se podría considerar su <<versión semisupervisada>>. 
...
\begin{itemize}
	...
	\item $\mathbf{W}$: sumatorio de todas las confianzas de predicción individuales de un conjunto $W = \sum w_{i,t,j}$.
	...
	\item $\mathbf{U'_{i,t}}$: conjunto de U cuya $W$ es menor que $Wmax$ (consultar ecuación~\ref{eqn:Wmax}).
...
Se facilita a continuación la versión del \textit{co-forest} de Engelen~\cite{engelen2018thesis}, basada en~\cite{originalCoForest2007} con los cambios observables en la sección <<Parámetros del algoritmo>>~\ref{parag:Wmax_inicial}.
...
\begin{algorithm}
	...
	\For{$i$ = 0, ..., $n-1$}{
		...
		$W_{i,0} \leftarrow min(\frac{1}{10}|$U$|, 100)$ \tcc*[f]{Consultar~\ref{parag:Wmax_inicial}}\\
	}
	...
	\For{$i$ = 0, ..., $n-1$}{
		...
		\If{$\hat{e}_{i,t} < \hat{e}_{i,t-1}$}{
			...
			\ForEach{$x_j \in U'_{i,t}$}{
				...
				\If{Confidence($H_k, x_j$) > $\theta$}{
					$L'_{k,t} \leftarrow L'_{k,t} \cup {x_j, H_i(x_j)}$\\
